...
import { StyleSheet, Text, View, TouchableWithoutFeedback, useColorScheme, Image, Platform, Dimensions, TextInput, TouchableOpacity, ActivityIndicator } from 'react-native';
...
const InputField = (props) => {
    const { label, placeholder, inputValue, type, editable, onToogleOpen, keyboardType = 'default', displayItems, onChangeText, onChangeDateValue, prefix = true } = props;
    ...
    if (editable === false) {
        return (
            <View style={[styles.form_group, { borderWidth: 1, borderColor: focusColor ? primary_color : '#EEEEEE', flexDirection: 'row', alignItems: 'center', paddingLeft: 0, }]}>
                <View style={{ width: '87%', height: 48, justifyContent: 'space-evenly', paddingLeft: 20, paddingTop: 5, paddingBottom: 5, }}>
                    <Text style={[styles.form_group_label, { color: focusColor ? primary_color : '#545454', }]}>{label}</Text>
                    <TextInput
            ...
            <>
                <View style={[styles.form_group, {}]}>
                    ...
                    <TouchableOpacity
                        ...
                        <View style={{ width: '87%', height: 48, justifyContent: 'space-evenly', paddingLeft: 20 }}>
                ...
                <View style={[styles.form_group, { borderWidth: 1, borderColor: focusColor ? primary_color : '#EEEEEE', flexDirection: 'row', alignItems: 'center', paddingLeft: 0, }]}>
                    <View style={{ width: '87%', height: 48, justifyContent: 'space-evenly', paddingLeft: 20, paddingTop: 5, paddingBottom: 5, }}>
                        ...
                        <TextInput
                            ...
                            placeholderTextColor={"#AFAFAF"}
                            style={styles.form_group_input}
                            onChangeText={val => { setPassword(val); onChangeText(val) }}
                            onFocus={_onFocus}
                            onBlur={_onBlur}
                            secureTextEntry={secureTextEntry}
                        // {...props}
    ...
    if (type === 'web-search') {
        return (
            <View style={[styles.form_group, {
                height: 41, backgroundColor: '#F8F8F8', flexDirection: 'row', borderRadius: 12,
                alignItems: 'center', paddingLeft: 0, justifyContent: 'space-between'
            }]}>
                <View style={{ width: 30, left: 10, justifyContent: 'center', alignItems: 'center' }}>
                ...
                <View style={{ width: '86%', height: '100%', justifyContent: 'center', alignItems: 'center' }}>
                    <TextInput
                        ...
                        onChangeText={onChangeText}
                        placeholderTextColor={"#AFAFAF"}
                        style={[styles.form_group_input, { height: '100%', backgroundColor: '#F8F8F8', color: '#000', marginTop: 0, padding: 0 }]}
                        onFocus={_onFocus}
                        onBlur={_onBlur}
                        {...props}
                    />
    ...
    return (
        <View style={[styles.form_group,]}>
            ...
            <TextInput
                ...
                onChangeText={onChangeText}
                placeholderTextColor={"#AFAFAF"}
                style={[styles.form_group_input, { borderWidth: 1, borderColor: focusColor ? primary_color : '#EEEEEE', }]}
                onFocus={_onFocus}
                onBlur={_onBlur}
                editable={editable}
