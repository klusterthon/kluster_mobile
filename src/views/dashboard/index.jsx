...
export default function HomeScreen({ navigation }) {
    ...
    const DateCom = ({ item, isSelected, setSelectedDate }) => (
        <TouchableOpacity onPress={() => setSelectedDate(item)} style={[styles.dateCon, isSelected && styles.selectedDateCon]}>
            <Text style={[custom_styles.h3, { color: isSelected ? '#fff' : colors.text }]}>{formatDateToDayOfWeek(item)}</Text>
            <Text style={[custom_styles.h3, { color: isSelected ? '#fff' : colors.text }]}>{item?.getDate()}</Text>
        </TouchableOpacity>
    )
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', }}>
                    <View style={[custom_styles.card_list_circle, { backgroundColor: '#E1E1E1', borderRadius: 45, marginRight: 10 }]}></View>
                    <View>
                        <Text style={[custom_styles.h4, { color: colors.text }]}>Hello {dates?.length}</Text>
                        <Text style={[custom_styles.h2, { color: colors.text }]}>Jane David</Text>
                    </View>
                </View>
                <View>
                    <SvgXml
            ...
            <View style={styles.body}>
                <View style={[{ width: '100%', alignItems: 'center', justifyContent: 'center' }]}>
                    <Text style={[custom_styles.h2, { fontSize: 16, color: colors.text }]}>Today, Nov 20</Text>
                    <View style={{
                        width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 20
                    }}>
                        <TouchableOpacity onPress={() => removeDate()} style={{ width: '10%' }}>
                            <SvgXml
                        ...
                        <View style={{
                            width: '80%', flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'center'
                        }}>
                            {
                                dates?.map((item, index) => (
                                    <DateCom key={index} item={item} isSelected={item === selectedDate} setSelectedDate={setSelectedDate} />
                                ))
                            }
                        </View>
                        <TouchableOpacity onPress={() => addNewDate()} style={{ width: '10%' }}>
                            <SvgXml
                                xml={`<svg width="23" height="22" viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    ListEmptyComponent={() => (
                        <View style={{ width: '100%', height: 500, justifyContent: 'center', alignItems: 'center' }}>
                            <SvgXml
                                ...
                                `}
                            />
                            <Text style={[custom_styles.h4, { fontSize: 16, color: colors.text, textAlign: 'center' }]}>
                                Your medications will be diplayed here, add medicine to get started
                            </Text>
                            <TouchableOpacity
                                onPress={() => {
                                ...
                                <SvgXml
                                    ...
                                    `}
                                />
                                <Text style={[custom_styles.button_text, { marginLeft: 5 }]}>Add medicine</Text>
                            </TouchableOpacity>
                        </View>
                    )}
                />
