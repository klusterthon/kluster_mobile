...
const formatDateString = (date, type) => {
    if(type == 'time'){
        ...
        const formattedTime = `${twelveHourFormat}:${minutes.toString().padStart(2, '0')} ${timeFormat}`;
...
const getRandomBgColors = () => {
    const all_color = [ '#FCF1CE','#FCF1CE','#D5E2F6','#FCF1CE','#FFD0CF','#D5E2F6','#FECEEF','#D5E2F6','#D2F4E4' ];
...
const getPercentage = (currentAmount, EstimatedAmount) => {
    var totalPercentage = Math.round((Number(currentAmount) / Number(EstimatedAmount)) * 100);
